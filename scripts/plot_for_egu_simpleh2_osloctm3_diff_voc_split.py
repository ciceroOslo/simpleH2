...
frac_voc_org_fabien = 0.39  #Fabien
...
pam_dict_voc2 = pam_dict_voc1.copy()
pam_dict_voc2["frac_voc_org"] =frac_voc_org_fabien 
print(pam_dict_voc2)
print(pam_dict_voc1)

sh2_voc1 = SIMPLEH2(pam_dict=pam_dict_voc1 ,paths=paths)
sh2_voc2 = SIMPLEH2(pam_dict=pam_dict_voc2 ,paths=paths)


#Scale only the anthropogenic emissions to match the estimated emissions in the models.
sh2_voc1.scale_emissions_antr(df_budget.loc[model]['H2 estimated emissions [Tg/yr]'])
sh2_voc2.scale_emissions_antr(df_budget.loc[model]['H2 estimated emissions [Tg/yr]'])


sh2_voc1.calculate_concentrations(const_oh=1,startyr=startyr,endyr=endyr)
sh2_voc2.calculate_concentrations(const_oh=1,startyr=startyr,endyr=endyr)
...
tot_prod_voc1 = sh2_voc1.h2_prod_emis.loc[startyr:endyr].sum(axis=1) + nit_fix

tot_emis_voc1 = (sh2_voc1.h2_prod_emis["h2_antr"].loc[startyr:endyr] +
            sh2_voc1.h2_prod_emis["h2_bb_emis"].loc[startyr:endyr] +
            nit_fix)

tot_atm_prod_voc1= (sh2_voc1.h2_prod_emis["h2_prod_ch4"].loc[startyr:endyr] +
                sh2_voc1.h2_prod_emis["h2_prod_nmvoc"].loc[startyr:endyr])

    
#Plot emissions


axs[0].plot(sh2_voc1.h2_prod_emis["h2_antr"],linestyle=lstyle,color='blue', linewidth=lw,label='H$_2$ antr. emis.')
axs[0].plot(sh2_voc1.h2_prod_emis["h2_bb_emis"],linestyle=lstyle,color='forestgreen', linewidth =lw,label='BB. em. emis.')
axs[0].plot([startyr,endyr],[nit_fix,nit_fix],linestyle=lstyle, color='orange', linewidth =lw,label='Nit. fix. (ocean and land)')
axs[0].plot(sh2_voc1.h2_prod_emis["h2_prod_ch4"],linestyle=lstyle, color='purple',linewidth =lw,label='Atm. Prod. (CH4) - \nFraction: 0.44')
axs[0].plot(sh2_voc1.h2_prod_emis["h2_prod_nmvoc"],linestyle=lstyle, color='darkblue',linewidth =lw,label='Atm. Prod. (NMVOC) - \nFraction: 0.44')
#Plot total production and emissions.
axs[1].plot(tot_emis_voc1,linestyle=lstyle, linewidth=lw,color='darkblue',label='Total emissions')
axs[1].plot(tot_atm_prod_voc1,linestyle=lstyle, linewidth=lw, color='purple',label='Total atm. production - Frac = 0.44')

#Plot concentrations
axs[2].plot(sh2_voc1.conc_h2, linestyle=lstyle, color='darkgreen', linewidth=lw, label=f'SimpleH2 (Frac VOC/CH4 prod: {pam_dict_voc1["frac_voc_org"]:.2f})')

#VOC2
lstyle = '--'

tot_prod_voc2 = sh2_voc2.h2_prod_emis.loc[startyr:endyr].sum(axis=1) + nit_fix

tot_emis_voc2 = (sh2_voc2.h2_prod_emis["h2_antr"].loc[startyr:endyr] +
            sh2_voc2.h2_prod_emis["h2_bb_emis"].loc[startyr:endyr] +
            nit_fix)

tot_atm_prod_voc2= (sh2_voc2.h2_prod_emis["h2_prod_ch4"].loc[startyr:endyr] +
                sh2_voc2.h2_prod_emis["h2_prod_nmvoc"].loc[startyr:endyr])

    
#Plot emissions
#Changed by SK on June 5 2024 to add stippled lines legend

axs[0].plot(sh2_voc2.h2_prod_emis["h2_antr"],linestyle=lstyle,color='blue', linewidth=lw) #,label='H$_2$ antr. emis.')
axs[0].plot(sh2_voc2.h2_prod_emis["h2_bb_emis"],linestyle=lstyle,color='forestgreen', linewidth =lw) #,label='BB. em. emis.')
axs[0].plot([startyr,endyr],[nit_fix,nit_fix],linestyle=lstyle, color='orange', linewidth =lw) #,label='Nit. fix. (ocean and land)')
axs[0].plot(sh2_voc2.h2_prod_emis["h2_prod_ch4"],linestyle=lstyle, color='purple',linewidth =lw, label='Atm. Prod. (CH4) -  \nFraction : 0.39') #,label='Atm. Prod. (CH4)')
axs[0].plot(sh2_voc2.h2_prod_emis["h2_prod_nmvoc"],linestyle=lstyle, color='darkblue',linewidth =lw,label='Atm. Prod. (NMVOC) -  \n Fraction : 0.39') #,label='Atm. Prod. (NMVOC)')

#Plot total production and emissions.
axs[1].plot(tot_emis_voc2,linestyle=lstyle, linewidth=lw,color='darkblue') #,label='Total emissions')
axs[1].plot(tot_atm_prod_voc2,linestyle=lstyle, linewidth=lw, color='purple') #,label='Total atm. production')

#Plot concentrations
axs[2].plot(sh2_voc2.conc_h2, linestyle=lstyle, color='darkgreen', linewidth=lw, label=f'SimpleH2 (Frac VOC/CH4 prod: {pam_dict_voc2["frac_voc_org"]:.2f})')
...
plt.tight_layout()
plt.show()
#plt.savefig('fig_egu_simpleh2_osloctm3_diff_voc_split.pdf',dpi=300)
